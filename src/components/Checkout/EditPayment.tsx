...
import { addCard, updateCard } from '@/redux/features/cardSlice';
...
export default function EditPayment() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      const response = await axios.put(`/api/card/null/${values.id}`, {
      ...
      if (response.status === 200) {
        dispach(setShowCardFormEdit(false));
        resetForm();
        dispach(updateCard(values));
      }
    },
  });
  ...
  useEffect(() => {
    if (data && data.length > 0) {
      const newData = {
        ...
        security_code: data[0].security_code || 0,
        names: data[0].names || '',
        surnames: data[0].surnames || '',
        number_identification: data[0].number_identification || 0,
      ...
      >
        ...
        <div className='grid grid-cols-2 w-full'>
          <div className='flex justify-start items-center'>
            <button
              onClick={() => dispach(setShowCardFormEdit(false))}
